    ...
    Ret =
        case concuerror_instr:instrument_and_compile(Files, Options) of
            {ok, Bin} ->
                ...
                Result =
                    do_analysis(NewTarget, PreBound, Dpor, CycleInfo, Options),
...
do_analysis(Target, PreBound, Dpor, CycleInfo, Options) ->
    Self = self(),
    Fun = fun() -> scheduler_main(Target, PreBound, Self, Dpor, CycleInfo,
                                  Options) end,
...
scheduler_main(Target, PreBound, Parent, Dpor, CycleInfo, Options) ->
    %% fprof:trace(start),
